...
const Table: FC<Props> = ({ bookingDetails, setSpaceId, toggleRatingModal }) => {
        ...
        <tbody>
          {bookingDetails.map(booking => (
            ...
            >
              <th
                onClick={() =>
                  router.push(`/spaces/${booking.Space.slug.current}`)
                }
                className='px-6 underline text-blue-600 cursor-pointer py-4 font-medium whitespace-nowrap'
              >
                {booking.Space.name}
              </th>
              <td className='px-6 py-4'>{booking.Space.price}</td>
